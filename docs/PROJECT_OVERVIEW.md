...
docs/                       PRD.md, SETUP_NEW_DATABASE.md, ZOHO_WEBHOOK_SETUP.md,
                            purchase-order-template.{pdf,docx}, PROJECT_OVERVIEW.md
...
- **Helvetica is WinAnsi-encoded** — it cannot render `₹` or `…`. Everything goes through
  `ascii()`, and money is written as `Rs. `.
...
- `blank: true` renders an empty form (no status line, no date, no amounts) — that's how
  `docs/purchase-order-template.pdf` is generated, so the printed template can't drift from
...
`main` is at PR **#12**. Two branches are pushed and awaiting PRs:
...
| `docs/purchase-order-template.pdf` | Blank PO form, generated by the app itself |
| `docs/purchase-order-template.docx` | Editable Word version of the same form |
